...
Module : TMRPCEN10 frontend, 10 s values per rate.
DOG : Difference of Gaussian approach to AGC
...
class TMRPCEN10dog(nn.Module):
    """Trainable Multi-rate PCEN, optimized IIR, difference of Gaussian approach
    to AGC
    Args:
    ...
    def __init__(self, n_bands: int=128, alpha: float=0.8,
        ...
        super(TMRPCEN10dog, self).__init__()
    ...
    def forward(self, x):
        ...
        print(f'S1: {s_vals[0]}')
        print(f'S5: {s_vals[4]}')
        print(f'S10: {s_vals[9]}')

        # Storage for each PCEN computation
        layered_pcen = []
        m_layers = [x] # DOG, store previously smoothed M(t,f)
        ...
        for s in s_vals:
            ...
            # Difference of Gaussian AGC
            agc = m_layers[-1] /(self.eps + smoother)**alpha
            pcen_ = (agc + delta)**r - delta**r
            m_layers.append(smoother)
        ...
        del layered_pcen, s_vals, smoother, pcen_
