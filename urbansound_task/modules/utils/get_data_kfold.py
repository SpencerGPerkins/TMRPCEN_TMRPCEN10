...
Module : Retrieve Kfold data
"""
import pandas as pd


directories = {'audio_path':
               ['../../data/urban/audio/wav/'],
               'meta_path':['../../data/urban/k_fold_meta/']
               }
...
class getKfoldDATA:
    ...
    def get_folded_data(val_fold, folds):
        ...
        if val_fold in folds:
            ...
            training_lst = [pd.read_csv(fold_path+fold+'.csv') for fold in tr_folds]
            meta_trdata = pd.concat(training_lst, ignore_index=True)
            meta_vldata = pd.read_csv(fold_path+val_fold+'.csv')
